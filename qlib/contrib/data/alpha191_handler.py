...
def extra_feature():
    return [
        ("ht001", "-1 * Corr(Rank(Delta(Log($volume), 1)), Rank(($close - $open) / $open), 6)"),
        ...
        # 022 SMEAN 没有定义，实现不了
        # ("023", "Sma(If(Delta($close, 1) > 0, Std($close,20), 0),20,1)/(Sma(If(Delta($close, 1) > 0, Std($close,20),0),20,1)+Sma(If(Delta($close, 1) <=0,Std($close,20),0),20,1))*100"),
        ...
        # 49,50,51都巨长
        # ("052", "Sum(Greater(0,$high-Ref(($high+$low+$close)/3,1)),26)/Sum(Greater(0,Ref(($high+$low+$close)/3,1)-$low),26)"), # 原文写错了，盲猜L=low
        ...
        # ("094", "SUM(Sign(Delta($close, 1)) * $volume,30)"),
